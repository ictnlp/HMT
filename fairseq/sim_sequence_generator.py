...
class SimSequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        while step < max_len + 1:
            ...
            # mask out inputs
            # cur_encoder_outs=[EncoderOut(
            # encoder_out=encoder_outs[0][0][:min(num_read,src_len),:,:],  # T x B x C
            # encoder_padding_mask=encoder_outs[0][1][:,:min(num_read,src_len)],  # B x T
            # encoder_embedding=encoder_outs[0][2][:,:min(num_read,src_len),:],  # B x T x C
            ...
            if read is not None:
                ...
                cands = cands.clamp(0, encoder_outs[0][0].size(0))
