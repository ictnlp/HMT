...
def _main(cfg: DictConfig, output_file):
    ...
    for sample in progress:
        ...
        # if 15 not in sample['net_input']['src_lengths'].tolist():continue
        # if 0 not in sample["id"].tolist(): continue
        ...
        for i, sample_id in enumerate(sample["id"].tolist()):
            ...
            translation_results[sample_id]["states_output"] = tgt_dict.string(
                extra["states_output"][sample_id], show_eos=True
            )
            # pdb.set_trace()
            ...
            if align_dict is not None:
                ...
                if src_dict is not None:
                    ...
                    bpe_src_str = src_dict.string(src_tokens, show_eos=True)
                else:
                    src_str = ""
                if has_target:
                    ...
                    bpe_ref_str = tgt_dict.string(target_tokens, show_eos=True)
            ...
            for j, hypo in enumerate(hypos[i][: cfg.generation.nbest]):
                ...
                bpe_tgt_str = tgt_dict.string(hypo["tokens"].int().cpu(), show_eos=True)
    ...
    print("AL score: ", al)
    print("DAL score: ", dal)
    if cfg.generation.translation_save_dir is not None:
        np.save(cfg.generation.translation_save_dir, translation_results)
    # np.save("./attn/gen.wait5.base.norm.npy",src_norm)
    ...
    np.savetxt("head_X_2000.npy",head_dicts['hidden'][:2000,:].cpu().numpy())
    np.savetxt("head_label_2000.npy",np.asarray(head_dicts['label'][:2000]) )
    """
